...
export async function setLayer(id: string, on: boolean, volume = 0.6) {
  ...
  if (on) {
    if (!active[id]) {
      const bus = TONAL.has(id) ? getReverb() : getMaster();
      const { spatialOn } = useStore.getState();
      let out: any;
      const extra: any[] = [];
      if (spatialOn && (Tone as any).Panner3D) {
        const panner = new Tone.Panner3D({ panningModel: "HRTF", positionX: Math.cos(Math.random() * 6.28) * 3, positionY: (Math.random() - 0.5) * 2, positionZ: Math.sin(Math.random() * 6.28) * 3 }).connect(bus);
        const orbitX = new Tone.LFO({ frequency: 0.03 + Math.random() * 0.04, min: -3, max: 3 }).start();
        orbitX.connect(panner.positionX);
        const orbitZ = new Tone.LFO({ frequency: 0.02 + Math.random() * 0.03, min: -3, max: 3 }).start();
        orbitZ.connect(panner.positionZ);
        out = new Tone.Gain(0).connect(panner);
        extra.push(panner, orbitX, orbitZ);
      } else {
        out = new Tone.Gain(0).connect(bus);
      }
      const nodes = build(id, out);
      active[id] = { gain: out, nodes: [...nodes, ...extra] };
    }
