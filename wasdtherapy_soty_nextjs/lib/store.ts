...
export const useStore = create<State>((set) => ({
  ...
  completeRitual: () =>
    set((s) => {
      ...
      if (s.ritualDoneDay === today) return {} as Partial<State>;
      const y = new Date(Date.now() - 86400000).toLocaleDateString("en-CA");
      const streak = s.ritualDoneDay === y ? s.streak + 1 : 1;
      return { ritualDoneDay: today, streak, bestStreak: Math.max(s.bestStreak, streak), totalDays: s.totalDays + 1 };
    }),
